...
#kidhtml用来生产首页不同页链接地址
    def kidhtml(self,indexhtml):
        ...
        for inhtml in indexhtml:
            kid_html = re.findall('<a class="link--tile" href="(.*en)">', inhtml)
            for i in kid_html:
                kidhtml.append(i)
        return kidhtml
...
#source用来生产具体资源
    def finallsource(self,kidsource):
        ...
        for each in pic:
            ...
            fp = open('/Users/messi/Pictures/moma/' +  str(n)  + u'--'  + times[n] + ' ' + u':' + works[n] + '.jpg','wb')
...
if __name__ == '__main__':
    ...
    url = 'http://www.moma.org/collection/artists/229?direction=fwd&locale=en&page=1'
    moma = spider()
    index_links = moma.changepage(url,3)
